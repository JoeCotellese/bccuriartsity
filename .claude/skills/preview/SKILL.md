...
2. Check whether the preview is already running before starting another one. If
   something is already serving on port 8080, just point the user at it.
3. Start `npm start` in the background from the project root.
4. Wait until it reports the server address, then open `http://localhost:8080`
   in the browser with `open http://localhost:8080`.
...
- If port 8080 is busy with something that is not this site, say so plainly and
